...
/** 提交病假申请 */
export async function submitSickLeave(form: Record<string, string>): Promise<SubmitResult> {
  console.log('\n📡 [Mock API] 提交病假申请...');
  await delay(300);
  const sickId = mockId('SL');
  console.log(`✅ [Mock API] 病假申请提交成功，ID: ${sickId}`);
  const result: SubmitResult = { success: true, resultId: sickId, form };
  return result;
}

/** 发起病假审批 */
export async function startSickLeaveProcess(resultId: string, _form: Record<string, string>): Promise<StartProcessResult> {
